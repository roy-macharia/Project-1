...
async function loadJournals() {
    ...
    const res = await fetch(`http://localhost:3000/journals?userId=${currentUser.id}`);
...
async function loadCommunity() {
    const res = await fetch('http://localhost:3000/communityPosts');
...
function addComment(postId) {
    ...
    fetch(`http://localhost:3000/communityPosts/${postId}`)
        .then(res => res.json())
        .then(post => {
            post.comments = post.comments || [];
            post.comments.push({ author: currentUser.username, text });

            return fetch(`http://localhost:3000/communityPosts/${postId}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
...
function deletePost(postId) {
    ...
    fetch(`http://localhost:3000/communityPosts/${postId}`, { method: 'DELETE' })
        .then(() => loadCommunity());
}
...
async function loadTherapists() {
    const res = await fetch('http://localhost:3000/therapists');
...
async function loadBookings() {
    ...
    const res = await fetch(`http://localhost:3000/bookings?userId=${currentUser.id}`);
